...
router.post('/users/:id/ban', authenticate, requireAdmin, validateParams(idParamSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/users/:id/unban', authenticate, requireAdmin, validateParams(idParamSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.post('/posts/:id/feature', authenticate, requireAdmin, validateParams(idParamSchema), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { action } = featurePostSchema.parse(req.body);
    const post = action === 'feature' 
